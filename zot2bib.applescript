	...
	tell application "BibDesk"
		...
		tell theDoc
			...
			tell newPub
				...
				set value of field "author" to (do shell script "echo " & (quoted form of ((value of field "author") as text)) & " | tr -d '{}'")
				set value of field "journal" to (do shell script "echo " & (quoted form of ((value of field "journal") as text)) & " | tr -d '{}'")
				set value of field "title" to (do shell script "echo " & (quoted form of ((value of field "title") as text)) & " | tr -d '{}'")
				...
				if value of field "doi" is not equal to "" then
				...
				if pdfPath is not equal to "" then
					...
					activate
					choose from list optList with prompt "Do the following with article PDF for " & (cite key as string) default items {"Autofile"}
					if item 1 of result is "Autofile" then
						set autofilePDF to true
					else if item 1 of result is "Autofile and delete first page of PDF" then
						set autofilePDF to true
					...
					if (cleanPDF is not equal to "") or (delPageOne is not equal to "") then
				      ...
				      tempfoo=cleanPDFtmpfile;¬
				      TMPFILE=`mktemp /tmp/${tempfoo}.XXXXXX` || exit 1;¬
				      pdftk \"" & pdfPath & "\" cat " & delPageOne & " output $TMPFILE " & cleanPDF & " 2>&1;¬
				      cp $TMPFILE \"" & pdfPath & "\";¬
				      rm $TMPFILE"
